...
namespace IWM.Repositories
{
    ...
    public class CategoryRepository : ICategoryRepository
    {
        ...
        private async Task<IQueryable<CategoryDAO>> DynamicFilter(IQueryable<CategoryDAO> query, CategoryFilter filter)
        {
            ...
            query = query.Where(q => q.ParentId, filter.ParentId, DataContext.Category.AsNoTracking());
        ...
        private async Task<IQueryable<CategoryDAO>> OrFilter(IQueryable<CategoryDAO> query, CategoryFilter filter)
        {
            ...
            foreach (CategoryFilter CategoryFilter in filter.OrFilter)
            {
                ...
                queryable = queryable.Where(q => q.ParentId, CategoryFilter.ParentId, DataContext.Category.AsNoTracking());
        ...
        {
            Dictionary<CategoryOrder, LambdaExpression> CustomOrder = new Dictionary<CategoryOrder, LambdaExpression>()
            {
                { CategoryOrder.Image, (CategoryDAO x) => x.Image.Name  },
                { CategoryOrder.Parent, (CategoryDAO x) => x.Parent.Name  },
                { CategoryOrder.Status, (CategoryDAO x) => x.Status.Name  },
            };
