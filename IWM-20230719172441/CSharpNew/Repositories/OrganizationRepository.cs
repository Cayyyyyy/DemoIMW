...
namespace IWM.Repositories
{
    ...
    public class OrganizationRepository : IOrganizationRepository
    {
        ...
        private async Task<IQueryable<OrganizationDAO>> DynamicFilter(IQueryable<OrganizationDAO> query, OrganizationFilter filter)
        {
            ...
            query = query.Where(q => q.ParentId, filter.ParentId, DataContext.Organization.AsNoTracking());
        ...
        private async Task<IQueryable<OrganizationDAO>> OrFilter(IQueryable<OrganizationDAO> query, OrganizationFilter filter)
        {
            ...
            foreach (OrganizationFilter OrganizationFilter in filter.OrFilter)
            {
                ...
                queryable = queryable.Where(q => q.ParentId, OrganizationFilter.ParentId, DataContext.Organization.AsNoTracking());
        ...
        {
            Dictionary<OrganizationOrder, LambdaExpression> CustomOrder = new Dictionary<OrganizationOrder, LambdaExpression>()
            {
                { OrganizationOrder.Parent, (OrganizationDAO x) => x.Parent.Name  },
                { OrganizationOrder.Status, (OrganizationDAO x) => x.Status.Name  },
            };
