using DotNetCore.CAP;
using TrueSight.Common;
using IWM.Common;
using IWM.Entities;
using IWM.Repositories;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;
using IWM.Services.MOrganization;
using TrueSight;
using TrueSight.RabbitMQ.Configuration;

namespace IWM.Handlers
{
    public class OrganizationHandler : Handler
    {
        private readonly IUOW UOW;
        private readonly IOrganizationService OrganizationService;
        public OrganizationHandler(ICurrentContext CurrentContext, IRabbitManager RabbitManager, IUOW UOW, IOrganizationService OrganizationService)
        {
            this.CurrentContext = CurrentContext;
            this.RabbitManager = RabbitManager;
            this.UOW = UOW;
            this.OrganizationService = OrganizationService;
        }
        protected override void Initialize(CapHeader headers, object content)
        {
            Preload(headers, content);
            CurrentContext.LoadConfiguration(StaticParams.ConnectionManager);
            UOW.LoadConfiguration();
        }
        
        [CapSubscribe(MessageRoutingKey.OrganizationSync)]
        public async Task Sync([FromCap] CapHeader Headers, List<Organization> Organizations)
        {
            try
            {
                Initialize(Headers, Organizations);
                if (Organizations != null && Organizations.Count > 0)
                    await OrganizationService.BulkMerge(Organizations);
            }
            catch (Exception ex)
            {
                Log(ex, nameof(OrganizationHandler));
            }
        }

    }
}