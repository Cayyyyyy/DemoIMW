    ...
    public class ProvinceGroupingRepository : IProvinceGroupingRepository
    {
        ...
        private async Task<IQueryable<ProvinceGroupingDAO>> DynamicFilter(IQueryable<ProvinceGroupingDAO> query, ProvinceGroupingFilter filter)
        {
            ...
            if (filter.ParentId != null && filter.ParentId.HasValue)
            {
                if (filter.ParentId.Equal != null)
                {
                    ...
                    query = query.Where(q => q.Parent.Path.StartsWith(ProvinceGroupingDAO.Path));
                }
                ...
                {
                    List<ProvinceGroupingDAO> ProvinceGroupingDAOs = DataContext.ProvinceGrouping
                        .Where(o => o.DeletedAt == null).ToList();
                    List<ProvinceGroupingDAO> Parents = ProvinceGroupingDAOs.Where(o => filter.ParentId.In.Contains(o.Id)).ToList();
                    List<ProvinceGroupingDAO> Branches = ProvinceGroupingDAOs.Where(o => Parents.Any(p => o.Path.StartsWith(p.Path))).ToList();
                ...
                if (filter.ParentId.NotIn != null)
                {
                    List<ProvinceGroupingDAO> ProvinceGroupingDAOs = DataContext.ProvinceGrouping
                        .Where(o => o.DeletedAt == null).ToList();
                    List<ProvinceGroupingDAO> Parents = ProvinceGroupingDAOs.Where(o => filter.ParentId.NotIn.Contains(o.Id)).ToList();
                    List<ProvinceGroupingDAO> Branches = ProvinceGroupingDAOs.Where(o => Parents.Any(p => o.Path.StartsWith(p.Path))).ToList();
                    List<long> Ids = Branches.Select(o => o.Id).ToList();
        ...
        private async Task<IQueryable<ProvinceGroupingDAO>> OrFilter(IQueryable<ProvinceGroupingDAO> query, ProvinceGroupingFilter filter)
        {
            if (filter.OrFilter == null || filter.OrFilter.Count == 0)
                return query;
            IQueryable<ProvinceGroupingDAO> initQuery = query.Where(q => false);
            foreach (ProvinceGroupingFilter ProvinceGroupingFilter in filter.OrFilter)
            {
                ...
                if (ProvinceGroupingFilter.ParentId != null && ProvinceGroupingFilter.ParentId.HasValue)
                {
                    if (ProvinceGroupingFilter.ParentId.Equal != null)
                    {
                        ProvinceGroupingDAO ProvinceGroupingDAO = DataContext.ProvinceGrouping
                            .Where(o => o.Id == ProvinceGroupingFilter.ParentId.Equal.Value).FirstOrDefault();
                        queryable = queryable.Where(q => q.Parent.Path.StartsWith(ProvinceGroupingDAO.Path));
                    }
                    if (ProvinceGroupingFilter.ParentId.NotEqual != null)
                    {
                        ProvinceGroupingDAO ProvinceGroupingDAO = DataContext.ProvinceGrouping
                            .Where(o => o.Id == ProvinceGroupingFilter.ParentId.NotEqual.Value).FirstOrDefault();
                        queryable = queryable.Where(q => !q.Parent.Path.StartsWith(ProvinceGroupingDAO.Path));
                    }
                    if (ProvinceGroupingFilter.ParentId.In != null)
                    {
                        List<ProvinceGroupingDAO> ProvinceGroupingDAOs = DataContext.ProvinceGrouping
                            .Where(o => o.DeletedAt == null).ToList();
                        List<ProvinceGroupingDAO> Parents = ProvinceGroupingDAOs.Where(o => ProvinceGroupingFilter.ParentId.In.Contains(o.Id)).ToList();
                        List<ProvinceGroupingDAO> Branches = ProvinceGroupingDAOs.Where(o => Parents.Any(p => o.Path.StartsWith(p.Path))).ToList();
                        List<long> Ids = Branches.Select(o => o.Id).ToList();
                        IdFilter IdFilter = new IdFilter { In = Ids };
                        queryable = queryable.Where(q => q.ParentId, IdFilter);
                    }
                    if (ProvinceGroupingFilter.ParentId.NotIn != null)
                    {
                        List<ProvinceGroupingDAO> ProvinceGroupingDAOs = DataContext.ProvinceGrouping
                            .Where(o => o.DeletedAt == null).ToList();
                        List<ProvinceGroupingDAO> Parents = ProvinceGroupingDAOs.Where(o => ProvinceGroupingFilter.ParentId.NotIn.Contains(o.Id)).ToList();
                        List<ProvinceGroupingDAO> Branches = ProvinceGroupingDAOs.Where(o => Parents.Any(p => o.Path.StartsWith(p.Path))).ToList();
                        List<long> Ids = Branches.Select(o => o.Id).ToList();
                        IdFilter IdFilter = new IdFilter { NotIn = Ids };
                        queryable = queryable.Where(q => q.ParentId, IdFilter);
        ...
        private IQueryable<ProvinceGroupingDAO> DynamicOrder(IQueryable<ProvinceGroupingDAO> query, ProvinceGroupingFilter filter)
        {
            switch (filter.OrderType)
            {
                case OrderType.ASC:
                    ...
                    {
                        case ProvinceGroupingOrder.Id:
                            query = query.OrderBy(q => q.Id);
                            break;
                        case ProvinceGroupingOrder.Code:
                            query = query.OrderBy(q => q.Code);
                            break;
                        case ProvinceGroupingOrder.Name:
                            query = query.OrderBy(q => q.Name);
                            break;
                        case ProvinceGroupingOrder.Status:
                            query = query.OrderBy(q => q.StatusId);
                            break;
                        case ProvinceGroupingOrder.Parent:
                            query = query.OrderBy(q => q.ParentId);
                            break;
                        case ProvinceGroupingOrder.HasChildren:
                            query = query.OrderBy(q => q.HasChildren);
                            break;
                        case ProvinceGroupingOrder.Level:
                            query = query.OrderBy(q => q.Level);
                            break;
                        case ProvinceGroupingOrder.Path:
