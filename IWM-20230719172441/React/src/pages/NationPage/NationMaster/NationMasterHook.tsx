...
import { NATION_ROUTE } from "config/routes";

/* end individual import */

export function useNationMasterHook() {
    const [translate] = useTranslation();
    const [visible, setVisible] = React.useState(false);
    const [modelFilter, dispatch, countFilter] =
        queryStringService.useQueryString(NationFilter, { skip: 0, take: 10 });
    ...
    const {
        ...
        handleResetList,
        handleLoadList,
    } = listService.useList(nationRepository.list, nationRepository.count, filter, dispatch);
    ...
    const {
        ...
        selectedRowKeys,
        setSelectedRowKeys
    } = listService.useRowSelection(nationRepository.delete, nationRepository.bulkDelete, null, null, null, handleResetList);
    ...
    const {
     ...
     const {
      ...
      } = masterService.useMasterAction(NATION_ROUTE, handleAction, handleBulkAction);
    ...
    const {
      ...
      const handleClickClearAllFilter = React.useCallback(() => {
        handleChangeAllFilter(new NationFilter());
      }, [handleChangeAllFilter]);
          ...
          const list: ListOverflowMenu[] = [
            {
              ...
              action:   ()=>handleOpenDetailModal(nation?.id),
              isShow: true,
            },
            {
            ...
            },
            {
              ...
              action: handleDeleteItem(nation),
              isShow: true,
            },
          ];
                    ...
                    
                    {
                        title: ({ sortColumns }) => {
                              ...
                              return (
                                <div>
                                    <LayoutHeader orderType='left'
                                                  title={translate("nations.priority")}
                                                  sortedColumn={sortedColumn}
                                                  isSorter />
                                </div>
                              );
                        ...
                        ellipsis: true,
                        render(...params: [number, Nation, number]) {
                            return (
                                <LayoutCell orderType="left" tableSize="md">
                    ...
                    
                    {
                        title: ({ sortColumns }) => {
                              ...
                              return (
                                <div>
                                    <LayoutHeader orderType='left'
                                                  title={translate("nations.createdAt")}
                                                  sortedColumn={sortedColumn}
                                                  isSorter />
                                </div>
                              );
    ...
    const listActions: any = [
        ...
        },
        {
          title: "Tải xuống",
          isShow: true,
          action: handleListExport(filter, nationRepository.export),
        },
        {
          title: "Tải xuống Template",
          isShow: true,
          action: handleExportTemplateList(nationRepository.exportTemplate),
        },
      ];
    return {
        visible,
